...
class ProcessUtils {
    ...
    public static boolean isServiceRun(Context mContext, String className) {
        ...
        for (int i = 0; i < size; i++) {
            if (serviceList.get(i).service.getClassName().equals(className) && serviceList.get(i).service.getPackageName().equals(AppContext.INSTANCE.getAppContext().getPackageName())) {
                isRun = true;
                break;
            }
        }
